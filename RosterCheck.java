...
//        }
        ...
        HashMap<String, Integer> countOff = new HashMap<String,Integer>();
        for (Roster roster : rosters) {
            String off = "" + roster.getOfferingID();
            if (countOff.get(off) == null) {
                countOff.put(off, 1);
            } else {
                countOff.put(off, countOff.get(off) + 1);
            }

        }
        for (String key : countOff.keySet()) {
            System.out.println("offering: " + key + " count: " + countOff.get(key));
        }
        System.out.println(countOff.size());
